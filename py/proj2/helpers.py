...
def write_csv(path_annotation: str, path_directory: str) -> None:
    ...
    data = []
    directory_name = path_directory.split('\\')[-1]

    for image_name in os.listdir(path_directory):
        absolute_path = os.path.join(path_directory, image_name)
        relative_path = os.path.join(directory_name, image_name)
...
def download_images(ranges: tuple[tuple[tuple[int, int], tuple[int, int]]], path_d: str) -> None:
    """
